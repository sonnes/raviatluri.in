...
Card.Title = function CardTitle({
  ...
  return (
    <Component className="text-base font-semibold tracking-tight text-text-primary">
...
Card.Description = function CardDescription({ children }: { children: React.ReactNode }) {
  return <p className="relative z-10 mt-2 text-sm text-text-secondary">{children}</p>;
};
...
Card.Eyebrow = function CardEyebrow({
  ...
  return (
    <Component
      className={clsx(
        className,
        'relative z-10 order-first mb-3 flex items-center text-sm text-text-muted',
        decorate && 'pl-3.5'
      )}
      {...props}
    >
