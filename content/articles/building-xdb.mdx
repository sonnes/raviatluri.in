...
xdb is a Go SDK that offers an abstraction layer over multiple databases in order to simplify data modeling, ingestion, querying, and operations.
...
A node is a logical entity uniquely identified by `Kind` and `ID`, both combined to form a `Key`.

The Subject of an N-Quad is the `Key` of the node.

## Attribute

Attributes have a `name` and a `value`. Attributes belong to a node. An attribute can be uniquely identified by the `Key` and `name`.

The Predicate of an N-Quad is the `name` of the attribute. The `value` is the Object.

## Value

xdb supports all Go primitive types as values. Values can be scalars or lists. Internally, xdb represents all values as bytes.

Custom types are supported by implementing the `Value` interface.

## Edge

Edges represent directed relationships between nodes. Think of an edge as a attribute whose value is a `Key` of target node. Edges can have additional metadata.

An edge can be uniquely identified by combination of source node `Key`, edge `name`, and target node `Key`.
