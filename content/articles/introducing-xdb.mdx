...
XDB is built around a fundamental building block - **Tuple**

### Tuple

A **Tuple** combines a id, attribute, value, and optional metadata.
...
```go
tuple := xdb.NewTuple("9bsv0s5ocl6002kdg0fg", "title", "Hello World")
```

### Edge

An **Edge** is a special kind of tuple whose value is a reference. Edges are unidirectional and represent relationships between entities.

### Record

A **Record** is a collection of tuples that share the same id. Records are similar to objects, structs, or rows in a database. Records represent entities in the domain model.
